 ...
 * @LastEditTime: 2021-12-24 17:44:53
 */
...
class Map extends Component {
    ...
    AMapLoader.load({
      ...
      .then((AMap) => {
        ...
        AMap.plugin(
          ...
          searchLngLat('start_input', 'startLngLat'),
        );
        //搜索终点地点
        AMap.plugin(
          ['AMap.PlaceSearch', 'AMap.AutoComplete'],
          searchLngLat('end_input', 'endLngLat'),
        );
        //搜索中心点设置
        !search_flag &&
          AMap.plugin(
            ['AMap.PlaceSearch', 'AMap.AutoComplete'],
            searchLngLat('searchBox_inp_input', 'searchLocation'),
          );
        //搜索关键字
        AMap.plugin(
          ['AMap.PlaceSearch', 'AMap.AutoComplete'],
          searchLngLat('keywordBox_inp', 'endLngLat'),
        );
      })
      .catch((e) => {
        console.error(e);
  ...
  clickLocation = () => {
    ...
    //为结果弹窗添加类显示
    this.locator.className = `${style.locator} ${style.active}`;
    this.setState({
      timerId: setTimeout(() => {
        //隐藏定位结果弹窗
        this.locator.className = `${style.locator}`;
      }, 5000),
    });
  };
      ...
      .then((AMap) => {
        ...
        AMap.plugin(['AMap.PlaceSearch'], () => {
          ...
          AMap.Event.addListener(
            ...
            'selectChanged',
            ({ selected: { data } }) => {
              let { lng, lat } = data.entr_location
                ? data.entr_location
                : data.location;
              //设置终点经纬度
              this.setState({
  ...
  render() {
    ...
    return (
      <div className={style.map}>
        ...
        {/* 功能盒子 -------------------------------------------------------------*/}
        <div
          className={`${style.typeBox} ${style.type} ${
            activeType === 'typeBox_block' ? style.typeBox_block : ''
          }`}
        >
          <ArrowLeftOutlined
            className={style.goProject}
        ...
        {/* 起点终点输入组件，根据起点终点规划路线 ------------------------------------------------------------------*/}
        <div
          className={`${style.mapLabel} ${style.locationSearch} ${
            activeType === 'navigationBox_block'
              ? style.navigationBox_block
              : ''
          }`}
        >
          <div className={style.startLocation}>
            <input type="text" id="start_input" placeholder="请输入起始地名" />
          </div>
          <div className={style.endLocation}>
            <input type="text" id="end_input" placeholder="请输入终点地名" />
          </div>
          <div className={style.buttonLocation} onClick={this.Get_Directions}>
            开始规划路线
          </div>
        </div>
        <button className={style.mapLabel} id="callApp"></button>
        {/* 搜索附近 ----------------------------------------------------------------------------------------------*/}
        <div
          className={`${style.mapLabel} ${style.searchBox} ${
            activeType === 'searchBox_block' ? style.searchBox_block : ''
          }`}
        >
          <div className={style.searchBox_inp}>
            <SendOutlined className={style.searchBox_inp_icon} />
            <input
              ...
              placeholder={
                search_flag ? '请输入搜索关键字' : '请输入要切换的地点'
              }
            />
            <SwapOutlined
              className={style.searchBox_inp_Swap}
              onClick={this.swapClick}
            />
          </div>
          <div className={style.searchBox_type}>
            {searchList[0] &&
              searchList.map((item) => (
                <div
                ...
                >
                  <span className={style.searchItem_icon}>{item.tag}</span>
                  <span className={style.searchItem_text}>{item.text}</span>
                </div>
              ))}
          </div>
          <div className={style.panel} id="panel"></div>
          <div className={style.searchBox_goSite} onClick={this.Get_Directions}>
            <BranchesOutlined />
            去这里
          </div>
        </div>
        {/* 关键字搜索 --------------------------------------------------------------------------------------------*/}
        <div
          className={`${style.mapLabel} ${style.keywordBox} ${
            activeType === 'keywordBox_block' ? style.keywordBox_block : ''
          }`}
        >
        ...
        {/* 区域抓取 ----------------------------------------------------------------------------------------------*/}
        <div
          className={`${style.mapLabel} ${style.areaBox} ${
            activeType === 'areaBox_block' ? style.areaBox_block : ''
          }`}
        ></div>
        {/* 定位结果弹窗 ------------------------------------------------------------------------------------------*/}
        <div
          className={style.locator}
          ref={(c) => (this.locator = c)}
          id="locator"
        >
          <div id="status"></div>
          <div id="result"></div>
